...
export class FoodEvaluationService {
   ...
   */
  public async evaluateFoodByWeather(
    request: IFoodEvaluationRequest,
  ): Promise<IFoodEvaluationResponse> {
    ...
    try {
      ...
      const weatherConditions =
        this.weatherAnalysisService.analyzeWeatherForFoodRecommendation(
          weatherData,
        );

      // 3. 모든 음식 카테고리 점수 계산
      const allScoredCategories =
        this.foodScoringService.calculateAllFoodScores(weatherConditions);

      // 4. 상위 3개 카테고리 선택
      const topCategories = this.foodScoringService.selectTopCategories(
        allScoredCategories,
        3,
      );
      ...
      return this.evaluateWithDefaultWeather(
        request,
        evaluationStartTime,
        error,
      );
  ...
  private evaluateWithDefaultWeather(
    ...
    // 기본 조건으로 점수 계산
    const allScoredCategories = this.foodScoringService.calculateAllFoodScores(
  ...
  public async getWeatherConditionsForLocation(location: ILatLng) {
    try {
      ...
      return this.weatherAnalysisService.analyzeWeatherForFoodRecommendation(
        weatherData,
      );
    } catch (error) {
      throw new Error(
        `날씨 조건 조회 실패: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
      );
    }
  }
   ...
   */
  public calculateFoodScoresForWeather(
    weatherConditions: any,
    topN: number = 3,
  ) {
    const allScoredCategories =
      this.foodScoringService.calculateAllFoodScores(weatherConditions);
    return this.foodScoringService.selectTopCategories(
      allScoredCategories,
      topN,
    );
  }
   ...
   */
  public generateEvaluationSummary(
    evaluationResult: IFoodEvaluationResponse,
  ): string {
    const { weather, topCategories } = evaluationResult;

    const weatherSummary =
      this.weatherAnalysisService.generateWeatherSummary(weather);
    const topFoodNames = topCategories.map((c) => c.nameKo).join(", ");

    const tempInfo = weather.actualTemperature
      ? `${weather.actualTemperature}°C`
      : "";
    const humidityInfo = weather.actualHumidity
      ? `습도 ${weather.actualHumidity}%`
      : "";
    const actualWeatherInfo = [tempInfo, humidityInfo]
      .filter(Boolean)
      .join(", ");
